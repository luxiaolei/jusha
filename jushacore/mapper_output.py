...
'''
'''
This module contains all the classes to define the L{mapper_output} class. As the name says, it is used as a container for the output from the jushacore algorithm, and the methods for displaying and analyzing the jushacore output accept this class as the input format.
'''
import sys
...
from jushacore.draw_mapper_output import draw_2D, draw_scale_graph, save_scale_graph_as_pdf, \
...
class mapper_output:
    '''
    jushacore output class

    This class stores the information given by the jushacore algorithm
    in the following attributes:

      - B{info}: a dictionary. This is a freeform attribute. Use it to store
        any information like the data source, the jushacore parameters, the
      ...
      - B{simplices}: the higher-dimensional simplices in the jushacore output,
    ...
    def __repr__(self):
        '''
        Represent the data of the jushacore output by a string.
        '''
        return repr({
            'info':      self.info,
    ...
    def generate_graph(self, cover=None, verbose=False, min_sizes=()):
        ...
        for i, n in enumerate(self.nodes):
            assert len(n.level)==1, \
                'This method can only be used for the 1-dimensional jushacore.'
            if n.points.size>=min_nodesize:
                self.add_simplex((i,), len(n.points))
    ...
    def save_to_file(self, filename):
        '''
        Save the jushacore output to a Pickle file.
    ...
    def load_from_file(filename):
        '''
        Load a jushacore output structure from a Pickle file.
    ...
    def from_Graph(G):
        ...
        only makes sense for graphs which have been converted from jushacore
    ...
    def from_db( cursor, expr_id ):
        ...
        @param expr_id: experiment id for the jushacore experiment you want to retrieve from the db
        @type expr_id: int
    ...
    def remove_nodes(self, nodes, verbose=False):
        '''
        Remove nodes from the jushacore output.
    ...
    def nodes_from_path(self, filt):
        '''
        Apply a path through the dendrograms to a jushacore output.
        '''
        if filt.ndim == 1:
